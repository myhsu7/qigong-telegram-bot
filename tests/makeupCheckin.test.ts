...
assert.match(checkinView, /grid-template-columns: minmax\(0,1fr\) minmax\(0,1fr\)/);
assert.doesNotMatch(checkinView, /\.date-tabs \{ grid-template-columns: 1fr; \}/);
assert.match(checkinView, /className = 'date-tab-main'/);
assert.match(checkinView, /className = 'date-tab-sub'/);
assert.match(checkinView, /makeupHint\.hidden = profile\.canMakeupYesterday/);
